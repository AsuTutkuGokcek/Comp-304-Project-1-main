...
int process_command(struct command_t *command)
{
	...
	if(strcmp(command->name,"filesearch") == 0){
		...
		if(strcmp(command->name,"letters") == 0){
		    ...
		    if(child == 0){
				scanf("%s", input);
				//the loop checks the entire input for every letter in the word
				for(int i=0; i<strlen(input); i++){
					...
					temp[0]=input[i];
					//when the loop is done for one letter, it prints the amount of that letter in the entire word
		...
		if(strcmp(command->name,"cdh") == 0){
			...
			/*			
			Implementation idea:
			We first thought about using a stack or queue to keep the visited directories.
			However, using stack or queue will not let us keep the results between instances which is required for this command.
			Therefore, we should either store the results in a file, or use environment variables which I will work on until deadline.
			*/
		}
